...
# Init-ordering shim (QA #1123): a strong LLVMFuzzerInitialize that waits for the asynchronously
# started Go c-archive runtime to finish registering its coverage counters before libFuzzer
# touches them (otherwise ~0.1% of starts SEGV in TracePC::ClearInlineCounters). See the file
# header. Linked BEFORE the archive so the archive resolves _cgo_wait_runtime_init_done.
$CC $GO_DEBUG_FLAGS -c "$SRC/mayhem/go_runtime_ready.c" -o "$SRC/mayhem-build/go_runtime_ready.o"
# Link: DWARF3 via $GO_DEBUG_FLAGS ensures the C-shim CU (first in the binary) is at DWARF3.
$CXX $SANITIZER_FLAGS $LIB_FUZZING_ENGINE $GO_DEBUG_FLAGS "$SRC/mayhem-build/go_runtime_ready.o" \
    "$SRC/mayhem-build/fuzz_json.a" -o /mayhem/fuzz_json
# Fail loudly if a toolchain change ever drops the shim. (No `grep -q`: under pipefail it would
# close the pipe early, nm would die of SIGPIPE and the check would fail spuriously.)
nm /mayhem/fuzz_json | grep ' T LLVMFuzzerInitialize$' >/dev/null \
    || { echo "build.sh: LLVMFuzzerInitialize init-ordering shim missing from /mayhem/fuzz_json" >&2; exit 1; }
